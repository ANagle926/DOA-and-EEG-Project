...
def apply_eemd_to_wave(wave, visualize=True):
    ...
    # Handle edge cases
    if kept_imfs.shape[0] < 9:
        # Pad with zeros if less than 9 IMFs
        pad = np.zeros((9 - kept_imfs.shape[0], wave.shape[0]))
        kept_imfs = np.vstack([kept_imfs, pad])

    # Transpose to shape (125, 9) for each sample
    return kept_imfs[:9].T

def _disable_ide_display():
    import os
    # Kill PyCharm/Datalore display env so workers don't try to POST back
    for k in ("PYCHARM_DISPLAY_PORT", "PYCHARM_HOSTED", "DLIS_SERVER_URL", "DLIS_SERVER_PORT"):
        os.environ.pop(k, None)
    # Make matplotlib/headless safe (in case EEMD plots internally)
    try:
        import matplotlib
        matplotlib.use("Agg", force=True)
    except Exception:
        pass
    # No-op IPython display calls, just in case
    try:
        ...
        pass
    # Avoid BLAS oversubscription inside each process
    os.environ.setdefault("OMP_NUM_THREADS", "1")
    os.environ.setdefault("OPENBLAS_NUM_THREADS", "1")
    os.environ.setdefault("MKL_NUM_THREADS", "1")


def process_dataset_eemd(x_data, n_jobs=8, use_threads=True):
    ...
    def safe_process(wave, i):
        try:
            ...
            # Avoid noisy prints from workers; they can trigger IDE hooks
            return np.zeros_like(wave.squeeze())

    backend = 'threading' if use_threads and n_jobs > 1 else 'loky'
    if n_jobs <= 1:
        processed = [safe_process(w, i) for i, w in enumerate(x_data)]
    else:
        processed = Parallel(n_jobs=n_jobs, backend=backend, verbose=0)(
