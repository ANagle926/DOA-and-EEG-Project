...
processed_model.save("eeg_regressor_150.keras")
evaluate_model(processed_model, x_test_processed, y_test_processed)

#pruning processed data
important_channels_processed, timestep_masks_processed = IG_temporal_and_spatial_importance(
    processed_model, x_train_processed[:2000],
    channel_threshold=0.15,
    timestep_percentile=60
)
x_train_processed_pruned = apply_temporal_and_spatial_pruning(x_train_processed, important_channels_processed, timestep_masks_processed)
x_test_processed_pruned = apply_temporal_and_spatial_pruning(x_test_processed, important_channels_processed, timestep_masks_processed)


#model trained on EEMD processed pruned data
model = build_model(x_train_processed_pruned, y_train_processed, x_test_processed_pruned, y_test_processed)
model.save("eeg_regressor_pruned_150.keras")
evaluate_model(model, x_test_processed_pruned, y_test_processed)
...
#ensemble model with EEMD pruned data
EEMD_pruned_preds= create_ensemble(x_train_processed_pruned, y_train_processed, x_test_processed_pruned, y_test_processed, n_models=n_models)
EEMD_pruned_meta_test_preds, EEMD_pruned_gbrt_model = create_GBRT(EEMD_pruned_preds, y_test_processed)

mae = mean_absolute_error(y_test_processed, EEMD_pruned_meta_test_preds)
mse = mean_squared_error(y_test_processed, EEMD_pruned_meta_test_preds)
corr = np.corrcoef(y_test_processed, EEMD_pruned_meta_test_preds)[0, 1]
r2 = r2_score(y_test_processed, EEMD_pruned_meta_test_preds)
