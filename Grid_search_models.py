...
def process_data(x_train, x_test, y_train, y_test, smote):
    ...
    continuous_feats = ["bmi", "creatine", "gpt"]          # real-valued
    binary_feats     = ["gender", "anemia", "hypertension", "diabetes"] + surg_cols  # already 0/1
    ...
    preprocessor = ColumnTransformer([
        ("scale", StandardScaler(), continuous_feats),   # centre/scale only real numbers
        ("bin",   "passthrough",    binary_feats)        # keep 0/1 flags unchanged
    ])
...
models = {
    ...
    #"CBWeighted":CatBoostClassifier(iterations=200, auto_class_weights=weights,random_seed=42, verbose=False),
    "LGBBalanced": LGBMClassifier(n_estimators=200, class_weight="balanced", random_state=42, verbose=-1),
    "LGBMWeighted": LGBMClassifier(n_estimators=200, class_weight=weights, random_state=42, verbose=-1),
    ...
    "RandomForestWeighted": RandomForestClassifier(n_estimators=200,max_depth=6, class_weight=weights,random_state=42,n_jobs=-1),
    ...
    "SGDWeighted" : SGDClassifier(loss="log_loss", penalty="elasticnet",class_weight=weights, random_state=42),
}
