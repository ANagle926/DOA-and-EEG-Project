...
def create_ensemble(x_train, y_train, x_test, y_test, n_models=5):
    ...
    for i in range(n_models):
        ...
        callbacks = [
            EarlyStopping(monitor='val_mae', patience=4, restore_best_weights=True, verbose=1),
        ]
        ...
        model.save(f'bootstrapping_raw{i}.keras')
...
def create_GBRT(n_models, x_test, y_test):
    ...
    for i in range(n_models):
        model = load_model(f'bootstrapping_raw{i}.keras')
        y_pred = model.predict(x_test, verbose=1)
        preds.append(y_pred)
        mae = mean_absolute_error(y_test, y_pred)
        val_maes.append(mae)
    ...
    }, "Saved Model Versions/Regressor/gbrt_model_bootstrap_raw.pkl")
...
for i in range(n_iterations):
    ...
    # fit model
    create_ensemble( x_bootstrap, y_bootstrap, x_test_raw, y_test_raw, n_models=n_models)
    mae, mse, rmse, corr, r2=create_GBRT(n_models=n_models, x_test=x_test_raw, y_test=y_test_raw)
